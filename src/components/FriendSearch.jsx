...
const FriendSearch = () => {
  ...
  useEffect(() => {
    const searchUsers = async () => {
      ...
      // FIX: userInfo se token nikalna
      const savedUser = localStorage.getItem("userInfo");
      const userInfo = savedUser ? JSON.parse(savedUser) : null;
      const token = userInfo?.token; 

      if (!token) {
        console.warn("Token missing from userInfo!");
        setLoading(false);
        return;
      }
          ...
          {query.trim() && (
            ...
            >
              ...
              {results.length > 0 ? (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-heading font-bold text-foreground truncate">{user.username}</p>
                        <p className="text-[10px] text-muted-foreground">
                          {isFriend ? "Already Friends" : isPending ? "Request Sent" : "New Discovery"}
                        </p>
                      </div>
                ...
                <p className="text-xs text-center text-muted-foreground py-4">No users found with that name 🌸</p>
              )}
            </motion.div>
          )}

          {/* Regular Friends List Section */}
          {!query.trim() && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              {friends?.length > 0 ? (
                friends.map((friend) => (
                   <div key={friend.id || friend._id} className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-muted/20 transition-all border border-transparent">
